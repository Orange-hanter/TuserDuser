...
func (s *EventService) CreateEvent(ctx context.Context, req *models.CreateEventRequest) (*models.Event, error) {
	...
	query := `
		...
		RETURNING id, type, start_time, end_time, duration, place, price_type, need_registration, details, created_at, updated_at
	`
	...
	s.logger.Info("Event created", zap.String("id", event.ID), zap.String("type", event.Type))
